...
基于 [go-music-dl](https://github.com/guohuiyuan/go-music-dl) 的音乐榜单、歌单、收藏夹和歌手关注监控下载服务。
...
内置榜单按平台分组，界面上可以先「校验」再「创建监控」：

| 平台 | 榜单来源 | 说明 |
|---|---|---|
| 网易云音乐 | 官方歌单 ID | 飙升榜、新歌榜、热歌榜、原创榜等 |
...
- 上游 go-music-dl 只有「歌单」能力，**没有榜单接口**；榜单页也不是歌单页。因此除网易云（榜单本身就是官方歌单）外，QQ / 酷狗 / 酷我的榜单由本服务直接读取各平台的公开榜单接口。
- 榜单 ID 会随平台改版失效，任意条目失效不影响其它功能，可在界面上「校验」自测。
- Apple Music 未内置：上游只能获取试听片段，本服务会过滤掉试听片段，因此无法得到完整音频。
- JOOX 未内置：其网页榜单页已下线，无法稳定解析；可用「自定义歌单」粘贴 JOOX 歌单链接代替。
- 任何平台都可以用「自定义榜单」手动新增歌单链接或歌单 ID，不受内置清单限制。

## 歌手关注

在监控弹窗里把类型选为「歌手关注」，每行填一个歌手名即可：
...
一行写「歌手名|平台1,平台2」可以只为这个歌手指定搜索平台；不写则用监控级勾选的全部平台。

工作原理：上游 go-music-dl **没有「按歌手取全部歌曲」的接口**，只有搜索。所以歌手关注的做法是
「按歌手名搜索 → 让引擎用 `exact_artist` 严格匹配歌手 → 本服务再按歌手名复核一遍」。
复核规则是歌手字段包含目标歌手即算（因此「周杰伦/杨瑞代」这类合作曲会保留），
而搜索结果里混进来的翻唱、同名曲（歌手字段完全对不上）会被剔除。

由于是搜索驱动，它的行为特点：

- 首次创建时会把该歌手**当前能搜到的曲目一次性抓下来**，之后每轮检查只下载新增的，不会重复。
- 新歌发布后会在下一轮检查时被发现（受平台的搜索收录速度影响，可能略有延迟）。
- 搜索能拿到多少首取决于平台搜索结果的数量上限（默认每歌手 100 首）。
- 与其它监控类型完全共享后续的增量比对、音质择优、跨平台换源与下载去重逻辑。
...
1. 进入「热门榜单」「歌单 / 收藏夹」，或直接在监控弹窗里选「歌手关注」
2. 选择平台和目标内容
3. 先点击预览，确认歌曲列表正确
4. 创建监控
5. 按需要打开自动下载
